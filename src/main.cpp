...
void setup(void) {
    ...
    // Print the header
    output =
        "Timestamp [ms], IAQ, IAQ accuracy, Static IAQ, CO2 equivalent, breath "
        "VOC equivalent, raw temp[°C], pressure [hPa], raw relative humidity "
        "[%], gas [Ohm], Stab Status, run in status, comp temp[°C], comp "
        "humidity [%], gas percentage";
    Serial.println(output);
}
